        ...
        <div className="bg-[white] p-6 rounded-2xl shadow-md border-t-4 border-[#FFE662]">
          ...
          <form onSubmit={accountForm.handleSubmit} className="space-y-4">
            {['name', 'email', 'phone'].map((field) => (
              <div key={field}>
                <label className="block mb-1 text-[#800020] capitalize">{field}</label>
                <input
                  type="text"
                  name={field}
                ...
                {accountForm.touched[field] && accountForm.errors[field] && (
                  <p className="text-red-500 text-sm mt-1">{accountForm.errors[field]}</p>
                )}
              </div>
            ))}
            ...
            {['address', 'city', 'postalCode'].map((field) => (
              <div key={field}>
                ...
                {addressForm.touched[field] && addressForm.errors[field] && (
                  <p className="text-red-500 text-sm mt-1">{addressForm.errors[field]}</p>
                )}
              </div>
            ))}
            <button type="submit" className="bg-[#800020] text-white px-6 py-2 rounded-xl hover:bg-[#a32638] transition">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-md border-t-4 border-[#FFE662]">
          ...
          <form onSubmit={passwordForm.handleSubmit} className="space-y-4">
            {['currentPassword', 'newPassword', 'confirmPassword'].map((field) => (
              <div key={field}>
                <label className="block mb-1 text-[#800020] capitalize">{field.replace(/([A-Z])/g, ' $1')}</label>
                <input
                  type="password"
                  name={field}
                  ...
                  className="w-full border rounded-xl p-2 focus:outline-none focus:ring-2 focus:ring-[#FFE662]"
                />
                {passwordForm.touched[field] && passwordForm.errors[field] && (
                  <p className="text-red-500 text-sm mt-1">{passwordForm.errors[field]}</p>
                )}
              </div>
            ))}
            <button type="submit" className="bg-[#800020] text-white px-6 py-2 rounded-xl hover:bg-[#a32638] transition">
